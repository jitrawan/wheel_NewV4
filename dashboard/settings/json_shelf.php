...
//$getvale = $getdata->my_sql_select(null,"shelf","shelf_header_code = '".addslashes($_GET['key'])."'");
...
,"shelf s"
,"s.shelf_header_code = '".addslashes($_GET['key'])."' ");

echo "<option value='' selected='selected'>--เลือกชั้นวางสินค้า--</option>";
while($row = mysql_fetch_array($getvale)) {
	$gettimt = $row['amt'] - $row['rimit'];
	echo "<option value=" . $row['shelf_code']  .">ชั้น" . $row['shelf_class'] . " เหลือ " .$gettimt. "</option>";
}
